...
export default async function handler(req, res) {
  ...
  if (req.method === "GET") {
    ...
    if (quote.verwijderdOp && quote.verwijderdReden === "foutief") {
      return json(res, 404, { ok: false, error: "Offerte niet gevonden." });
    }
    if (!quote.verwijderdOp && quote.status !== "concept") {
      try {
    ...
    const status = quote.verwijderdOp ? "afgewezen" : quote.status;
    return json(res, 200, { ok: true, quote: { nummer: quote.nummer, klantNaam: quote.klantNaam, totaal: quote.totaal, status, regels: quote.regels, geldigTot: quote.geldigTot } });
  }

  if (req.method === "POST") {
    const { token, actie } = req.body || {};
    const quote = await getQuoteByToken(token);
    if (!quote || (quote.verwijderdOp && quote.verwijderdReden === "foutief")) {
      return json(res, 404, { ok: false, error: "Offerte niet gevonden." });
    }
    if (quote.verwijderdOp) {
      return json(res, 400, { ok: false, error: "Deze offerte is niet meer geldig." });
    }
    if (actie === "accepteer") {
