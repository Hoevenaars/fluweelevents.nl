import {
  listQuotes, getQuote, createQuote, updateQuote, deleteQuote, getTemplate, applyTemplate,
  QUOTE_DELETE_REASONS,
} from "../crm.js";
...
export default async function handler(req, res) {
  ...
  return withAuth(req, res, async () => {
    ...
    if (req.method === "DELETE") {
      const { id, reden } = req.body || {};
      if (!id) return json(res, 400, { ok: false, error: "id verplicht." });
      if (!QUOTE_DELETE_REASONS.includes(reden)) {
        return json(res, 400, { ok: false, error: "Kies waarom: foutief of afgewezen." });
      }
      try {
        const quote = await deleteQuote(id, reden);
        if (!quote) return json(res, 404, { ok: false, error: "Niet gevonden." });
        return json(res, 200, { ok: true, quote });
      } catch (err) {
        return json(res, err.status || 500, { ok: false, error: err.message || "Verwijderen mislukt." });
      }
    }
