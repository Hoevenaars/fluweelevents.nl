...
import { calcVerkoopprijs, money, normalizeProjectCostInput, splitCostStatus } from "./project-calc.js";

const __dirname = dirname(fileURLToPath(import.meta.url));
const ROOT = join(__dirname, "..");
const DEFAULT_CRM_FILE = join(ROOT, ".data", "crm.json");
const LEGACY_FILE = join(ROOT, ".data", "submissions.json");

function crmFilePath() {
  return process.env.FLUWEEL_CRM_FILE || DEFAULT_CRM_FILE;
}
...
export const QUOTE_DELETE_REASONS = ["foutief", "afgewezen"];
...
// ===== LOCAL DB =====
function ensureLocalCollections(db) {
  ...
  if (!Array.isArray(db.projectCosts)) db.projectCosts = [];
  db.projectCosts = db.projectCosts.map((cost) => {
    const next = { ...cost, ...splitCostStatus(cost) };
    delete next.bevestigdBetaald;
    delete next.bevestigd_betaald;
    return next;
  });
  return db;
}
...
async function readLocalDb() {
  const crmFile = crmFilePath();
  if (!existsSync(crmFile) && existsSync(LEGACY_FILE)) {
  ...
  if (!existsSync(crmFile)) return emptyDb();
  try {
    const db = JSON.parse(await readFile(crmFile, "utf8"));
    return ensureLocalCollections(db);
  } catch {
    return emptyDb();
  }
}
...
async function writeLocalDb(db) {
  ...
  await writeFile(crmFile, JSON.stringify(db, null, 2), "utf8");
}
...
function mapQuote(row, lines = []) {
  return {
    ...
    bekekenAantal: Number(row.bekeken_aantal ?? row.bekekenAantal ?? 0),
    laatstBekekenOp: row.laatst_bekeken_op ?? row.laatstBekekenOp ?? null,
    bekekenOp: Array.isArray(row.bekeken_op ?? row.bekekenOp) ? (row.bekeken_op ?? row.bekekenOp) : [],
    verwijderdOp: row.verwijderd_op ?? row.verwijderdOp ?? null,
    verwijderdReden: row.verwijderd_reden ?? row.verwijderdReden ?? null,
...
function mapProjectCost(row) {
  const prijs = num(row.prijs);
  const margePct = num(row.marge_pct ?? row.margePct);
  const verkoopprijs = row.verkoopprijs != null && row.verkoopprijs !== ""
    ? num(row.verkoopprijs)
    : calcVerkoopprijs(prijs, margePct);
  return {
    id: row.id,
...
function mapProject(row, kosten = []) {
  return {
    id: row.id,
    leadId: row.lead_id ?? row.leadId,
    naam: row.naam,
    ...
    aangemaaktOp: row.aangemaakt_op ?? row.aangemaaktOp,
    kosten: kosten.map(mapProjectCost),
  };
}

function attachLocalProjectCosts(projects, db) {
  const byProject = new Map();
  for (const cost of db.projectCosts || []) {
    const list = byProject.get(cost.projectId) || [];
    list.push(cost);
    byProject.set(cost.projectId, list);
  }
  return projects.map((p) => {
    const kosten = (byProject.get(p.id) || [])
      .slice()
      .sort((a, b) => (a.sortOrder - b.sortOrder) || String(a.aangemaaktOp || "").localeCompare(String(b.aangemaaktOp || "")));
    return { ...p, kosten: kosten.map(mapProjectCost) };
  });
}
...
// ===== QUOTES =====
function withLocalQuoteFields(q, regels) {
  return {
    ...q,
    bekekenAantal: Number(q.bekekenAantal || 0),
    laatstBekekenOp: q.laatstBekekenOp || null,
    bekekenOp: Array.isArray(q.bekekenOp) ? q.bekekenOp : [],
    verwijderdOp: q.verwijderdOp || null,
    verwijderdReden: q.verwijderdReden || null,
    regels,
  };
}
...
export async function listQuotes() {
  ...
  const db = await readLocalDb();
  return db.quotes
    .filter((q) => !q.verwijderdOp)
    .map((q) => withLocalQuoteFields(
      q,
      db.quoteLines.filter((l) => l.quoteId === q.id).sort((a, b) => a.sortOrder - b.sortOrder),
    ))
...
export async function getQuote(id) {
  const quote = await loadQuoteRecord(id);
  if (!quote || quote.verwijderdOp) return null;
  return quote;
}
...
export async function getQuoteByToken(token) {
  const supabase = getServiceClient();
  if (supabase) {
    const { data, error } = await supabase.from("quotes").select("*").eq("portal_token", token).maybeSingle();
  ...
  if (!q) return null;
  return withLocalQuoteFields(q, db.quoteLines.filter((l) => l.quoteId === q.id));
}
...
export async function createQuote({ leadId, klantNaam, klantEmail, klantBedrijf, regels = [], geldigTot, notities = "" }) {
  ...
  const quote = {
    ...
    status: "concept", geldigTot, notities, portalToken, aangemaaktOp: nowIso(),
    bekekenAantal: 0, laatstBekekenOp: null, bekekenOp: [],
    verwijderdOp: null, verwijderdReden: null,
    ...totals, btwPct: totals.btw_pct, btwBedrag: totals.btw_bedrag, regels: [],
  };
...
export async function updateQuote(id, updates) {
  const current = await loadQuoteRecord(id);
  if (!current || current.verwijderdOp) return null;
  const supabase = getServiceClient();
  const patch = {};
...
export async function recordQuoteView(id) {
  const now = nowIso();
  const supabase = getServiceClient();
  if (supabase) {
    const current = await getQuote(id);
    if (!current) return null;
    const times = [...(current.bekekenOp || []), now].slice(-80);
    const { error } = await supabase.from("quotes").update({
      bekeken_aantal: (current.bekekenAantal || 0) + 1,
  ...
  const db = await readLocalDb();
  const idx = db.quotes.findIndex((q) => q.id === id);
  if (idx === -1 || db.quotes[idx].verwijderdOp) return null;
  const times = [...(db.quotes[idx].bekekenOp || []), now].slice(-80);
  db.quotes[idx] = {
    ...db.quotes[idx],
    bekekenAantal: (db.quotes[idx].bekekenAantal || 0) + 1,
    laatstBekekenOp: now,
    bekekenOp: times,
  };
  await writeLocalDb(db);
...
export async function deleteQuote(id, reden) {
  if (!QUOTE_DELETE_REASONS.includes(reden)) {
    const err = new Error("Kies waarom: foutief of afgewezen.");
    err.status = 400;
    throw err;
  }
  const current = await loadQuoteRecord(id);
  ...
  const patch = {
    verwijderd_op: now,
    verwijderd_reden: reden,
  };
  if (reden === "afgewezen") patch.status = "afgewezen";

  if (supabase) {
    const { error } = await supabase.from("quotes").update(patch).eq("id", id);
    if (error) throw error;
    return loadQuoteRecord(id);
  }
  const db = await readLocalDb();
  const idx = db.quotes.findIndex((q) => q.id === id);
  if (idx === -1) return null;
  db.quotes[idx] = {
    ...db.quotes[idx],
    verwijderdOp: now,
    verwijderdReden: reden,
    ...(reden === "afgewezen" ? { status: "afgewezen" } : {}),
  };
  await writeLocalDb(db);
  return loadQuoteRecord(id);
}
...
async function loadAllProjectCosts() {
  const supabase = getServiceClient();
  if (supabase) {
    const { data, error } = await supabase
      .from("project_costs")
  ...
  return (await readLocalDb()).projectCosts || [];
}

function groupCostsByProject(rows) {
  const byProject = new Map();
...
export async function listProjects() {
  ...
  if (supabase) {
    ...
    let costRows = [];
    try {
      costRows = await loadAllProjectCosts();
    } catch (err) {
      console.error("project_costs laden mislukt:", err?.message || err);
    }
    const byProject = groupCostsByProject(costRows);
    return (data || []).map((row) => mapProject(row, byProject.get(row.id) || []));
  }
  const db = await readLocalDb();
  return attachLocalProjectCosts(
    db.projects.sort((a, b) => new Date(b.aangemaaktOp) - new Date(a.aangemaaktOp)),
    db,
  );
}
...
export async function updateProject(id, updates) {
  ...
  if (supabase) {
    ...
    return mapProject(data, costRows);
  }
  const db = await readLocalDb();
  const idx = db.projects.findIndex((p) => p.id === id);
  if (idx === -1) return null;
  const safeUpdates = { ...updates };
  delete safeUpdates.kosten;
  db.projects[idx] = { ...db.projects[idx], ...safeUpdates };
  await writeLocalDb(db);
  return attachLocalProjectCosts([db.projects[idx]], db)[0];
}

function mergeCostUpdate(existing, updates) {
  const incoming = { ...mapProjectCost(existing), ...updates };
  if ((updates.prijs != null || updates.margePct != null || updates.marge_pct != null)
      && updates.verkoopprijs == null) {
    delete incoming.verkoopprijs;
  }
  return normalizeProjectCostInput(incoming);
}

export async function createProjectCost(projectId, input = {}) {
  if (!projectId) throw new Error("projectId verplicht.");
  const fields = normalizeProjectCostInput(input);
  const supabase = getServiceClient();
  if (supabase) {
    ...
    if (!project) {
      const err = new Error("Project niet gevonden.");
      err.status = 404;
      throw err;
    }
    ...
    const { data, error } = await supabase.from("project_costs").insert({
      ...
      wat: fields.wat,
      prijs: fields.prijs,
      marge_pct: fields.margePct,
      verkoopprijs: fields.verkoopprijs,
      opmerking: fields.opmerking,
      bevestigd: fields.bevestigd,
      betaald: fields.betaald,
      sort_order: sortOrder,
    }).select().single();
    if (error) throw error;
    return mapProjectCost(data);
  }
  ...
  const siblings = db.projectCosts.filter((c) => c.projectId === projectId);
  const sortOrder = siblings.reduce((max, c) => Math.max(max, Number(c.sortOrder || 0)), -1) + 1;
  const cost = {
    id: randomUUID(),
    projectId,
    ...fields,
  ...
  db.projectCosts.push(cost);
  await writeLocalDb(db);
  return mapProjectCost(cost);
}

export async function updateProjectCost(id, updates = {}) {
  if (!id) return null;
  const supabase = getServiceClient();
  if (supabase) {
    ...
    const merged = mergeCostUpdate(existing, updates);
    const { data, error } = await supabase.from("project_costs").update({
      leverancier: merged.leverancier,
      wat: merged.wat,
      prijs: merged.prijs,
      marge_pct: merged.margePct,
      verkoopprijs: merged.verkoopprijs,
      opmerking: merged.opmerking,
      bevestigd: merged.bevestigd,
      betaald: merged.betaald,
  ...
  const db = await readLocalDb();
  const idx = db.projectCosts.findIndex((c) => c.id === id);
  if (idx === -1) return null;
  const merged = mergeCostUpdate(db.projectCosts[idx], updates);
  db.projectCosts[idx] = {
    ...db.projectCosts[idx],
    ...merged,
  };
  await writeLocalDb(db);
  return mapProjectCost(db.projectCosts[idx]);
}
...
export async function deleteProjectCost(id) {
  ...
  const supabase = getServiceClient();
  if (supabase) {
    const { data, error } = await supabase.from("project_costs").delete().eq("id", id).select("id");
    if (error) throw error;
    return Boolean(data?.length);
  }
  const db = await readLocalDb();
  const next = db.projectCosts.filter((c) => c.id !== id);
  if (next.length === db.projectCosts.length) return false;
  db.projectCosts = next;
  await writeLocalDb(db);
  return true;
}
