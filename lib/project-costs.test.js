...
test("rekentoolregel hangt onder een project en herberekent verkoopprijs", async () => {
  ...
  assert.equal(cost.bevestigd, false);
  assert.equal(cost.betaald, false);
  ...
  const confirmed = await updateProjectCost(cost.id, { bevestigd: true });
  assert.equal(confirmed.bevestigd, true);
  assert.equal(confirmed.betaald, false);
  assert.equal(confirmed.verkoopprijs, 500);

  const paid = await updateProjectCost(cost.id, { betaald: true });
  assert.equal(paid.bevestigd, true);
  assert.equal(paid.betaald, true);
  ...
  assert.equal(raised.bevestigd, true);
  assert.equal(raised.betaald, true);
