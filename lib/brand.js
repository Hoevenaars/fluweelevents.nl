...
export const BRAND = {
  ...
  logo: {
    woordmerkZwart: "/logo/fluweel-woordmerk-zwart.png",
    woordmerkWit: "/logo/fluweel-woordmerk-wit.png",
    monogramZwart: "/logo/fluweel-monogram-zwart.png",
    monogramWit: "/logo/fluweel-monogram-wit.png",
    icoon: "/logo/fluweel-icon-192.png",
  },
  kleuren: {
    ivoor: "#FAF5EF",
    vlak: "#F3EAE0",
        ...
        <table role="presentation" width="560" cellpadding="0" cellspacing="0" style="max-width:560px;width:100%;background:${kleuren.wit};border:1px solid ${kleuren.lijn};">
          ...
          <tr>
            <td style="padding:28px 40px 8px;">
              <p style="margin:0;">
                <img src="${BRAND.site}${BRAND.logo.woordmerkZwart}" alt="Fluweel." height="28" style="display:block;height:28px;width:auto;border:0;">
              </p>
          ...
          <tr>
            <td style="padding:8px 40px 32px;border-top:1px solid ${kleuren.lijn};">
              ...
              <p style="margin:0;">
                <img src="${BRAND.site}${BRAND.logo.woordmerkZwart}" alt="Fluweel." height="22" style="display:block;height:22px;width:auto;border:0;">
              </p>
...
export function parseNlDateValue(iso) {
  if (!iso) return null;
...
const NL_DATE = { day: "numeric", month: "long", year: "numeric" };
const NL_DATETIME = { day: "numeric", month: "long", year: "numeric", hour: "2-digit", minute: "2-digit" };

export function formatNlDate(iso) {
  const date = parseNlDateValue(iso);
  if (!date) return iso ? String(iso) : "";
  try {
    return new Intl.DateTimeFormat("nl-NL", NL_DATE).format(date);
  } catch {
    return String(iso);
  }
}
...
export function formatNlDateTime(iso) {
  ...
  if (!date) return iso ? String(iso) : "";
  try {
    return new Intl.DateTimeFormat("nl-NL", NL_DATETIME).format(date);
  } catch {
    return String(iso);
  }
}
