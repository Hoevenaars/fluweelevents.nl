...
const LOGO_ZWART = join(dirname(fileURLToPath(import.meta.url)), "..", "logo", "fluweel-woordmerk-zwart.png");
...
function paintPage(doc) {
  ...
  doc.image(LOGO_ZWART, width - M.r - 92, height - 33, { height: 12 });
  doc.restore();
}

function writeMerk(doc, x, y) {
  doc.image(LOGO_ZWART, x, y, { height: 22 });
  return y + 34;
}
...
function writeRegels(doc, startY, regels = []) {
  ...
  doc.text("AANTAL", col.aantal, y, { width: 48, align: "right", characterSpacing: 1.2 });
  doc.text("PRIJS", col.prijs, y, { width: 78, align: "right", characterSpacing: 1.2 });
  ...
  for (const r of regels) {
    ...
    doc.text(r.omschrijving || "", col.omschr, rowTop, { width: 248 });
    const after = doc.y;
    doc.font("Helvetica").fontSize(10).fillColor(kleuren.inkt);
    doc.text(String(r.aantal ?? ""), col.aantal, rowTop, { width: 48, align: "right" });
    doc.text(euro(r.prijs), col.prijs, rowTop, { width: 78, align: "right" });
